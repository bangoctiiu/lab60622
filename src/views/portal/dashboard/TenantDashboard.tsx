...
const TenantDashboard: React.FC = () => {
  ...
  const contractExpiryDays = differenceInDays(new Date(data.activeContract.endDate), new Date());
      ...
      <div className="relative h-[280px] w-full overflow-hidden bg-gradient-to-br from-[#1B3A6B] via-[#0D8A8A] to-[#2E5D9F] px-8 pt-12">
        ...
        <div className="relative z-10 space-y-1">
          ...
          <h2 className="text-[36px] font-black text-white tracking-tighter leading-tight">
            Xin chào,<br/>
            <span className="italic">{data.activeContract.tenantName.split(' ').pop()}!</span>
          </h2>
        </div>
        ...
        <div className="bg-white/90 backdrop-blur-2xl rounded-[32px] p-6 shadow-premium border border-white/60 group hover:shadow-glow transition-all duration-500">
          <div className="flex items-center gap-5">
            ...
            <div className="flex-1 space-y-1">
              <div className="flex items-center justify-between">
                <p className="text-xl font-black text-slate-900 tracking-tight italic">Căn hộ {data.activeContract.roomCode}</p>
                <ChevronRight size={20} className="text-slate-300 group-hover:translate-x-1.5 transition-transform" />
              </div>
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{data.activeContract.buildingName}</p>
            </div>
          </div>
          ...
          <div className="mt-6 pt-5 border-t border-slate-100 flex items-center justify-between">
             <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider italic">Hợp đồng: {data.activeContract.endDate}</span>
             </div>
             <span className={cn(
