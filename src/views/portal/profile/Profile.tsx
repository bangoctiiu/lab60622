...
const Profile = () => {
  ...
  const { user, login, logout: authLogout } = useAuthStore();
  ...
  const handleLogout = async () => {
    ...
    if (ok) {
        authLogout();
        // Force a hard reload to login to clear all states reliably
  ...
  const updateAvatarMutation = useMutation({
    ...
    onSuccess: (res: any) => {
      ...
      if (user) {
        ...
        login({ ...user, avatar: res.data.avatarUrl }, localStorage.getItem('access_token') || '');
      }
  ...
  return (
    ...
    <div className="min-h-screen bg-slate-50 relative overflow-x-hidden animate-in fade-in slide-in-from-right-6 duration-700">
      ...
      <BottomSheet isOpen={activeSheet === 'personal'} onClose={() => setActiveSheet(null)} title="Định danh cư dân">
        <PersonalInfoForm profile={profile} onClose={() => setActiveSheet(null)} />
      </BottomSheet>

      {/* Security Hub Sheet */}
      <BottomSheet isOpen={activeSheet === 'security'} onClose={() => setActiveSheet(null)} title="Trung tâm bảo mật">
        <SecuritySettings onClose={() => setActiveSheet(null)} />
      </BottomSheet>

      {/* Feedback Sheet */}
      <BottomSheet isOpen={activeSheet === 'feedback'} onClose={() => setActiveSheet(null)} title="Góp ý trải nghiệm">
        <FeedbackForm onClose={() => setActiveSheet(null)} />
      </BottomSheet>

      {/* Notifications Sheet */}
      <BottomSheet isOpen={activeSheet === 'notifications'} onClose={() => setActiveSheet(null)} title="Kênh thông báo">
