...
const InvoiceList = () => {
  ...
  const tabs: { id: InvoiceStatus | 'All'; label: string; color: string }[] = [
    ...
    { id: 'Paid', label: 'Đã trả', color: 'success' },
    { id: 'Cancelled', label: 'Đã hủy', color: 'muted' },
  ];
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-display text-primary">Quản lý Hóa đơn</h1>
          <p className="text-body text-muted">Theo dõi công nợ, doanh thu và tình trạng thanh toán cư dân.</p>
        </div>
        <div className="flex items-center gap-3">
          <button className="btn-outline flex items-center gap-2" onClick={() => setIsBulkModalOpen(true)}>
            <Layers size={18} /> Tạo hàng loạt
          </button>
          ...
          <table className="w-full text-left border-collapse">
            <thead className="bg-muted/10 border-b border-border/50">
              <tr>
                ...
                <th className="px-6 py-4 text-label text-muted">Cư dân</th>
                <th className="px-6 py-4 text-label text-muted">Kỳ</th>
                <th className="px-6 py-4 text-label text-muted">Tổng tiền</th>
                <th className="px-6 py-4 text-label text-muted">Còn lại</th>
                <th className="px-6 py-4 text-label text-muted text-center">Trạng thái</th>
                <th className="px-6 py-4 text-label text-muted">Hạn TT</th>
                <th className="px-6 py-4 text-label text-muted text-center">Đã xem?</th>
