      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1 card-container p-8 border-2 border-danger/10 bg-danger/[0.02] relative overflow-hidden">
          ...
          <div className="flex items-center justify-between mb-8">
             ...
             <span className="px-3 py-1 bg-danger/10 text-danger text-[10px] font-black rounded-full uppercase tracking-tighter animate-pulse">
                {alerts?.length || 0} Issues
             </span>
          </div>
          ...
          <div className="space-y-4">
            {alerts?.map((alert) => (
              <div key={alert.id} className={cn(
                ...
                alert.severity === 3 ? "bg-white border-danger shadow-sm shadow-danger/5" :
                alert.severity === 2 ? "bg-white border-warning shadow-sm shadow-warning/5" : "bg-white border-primary shadow-sm shadow-primary/5"
              )}>
                <div className="flex justify-between items-start">
                  <span className={cn(
                  ...
                  <button 
                    ...
                    className="p-1.5 hover:bg-bg rounded-lg text-muted opacity-0 group-hover/alert:opacity-100 transition-all active:scale-75"
                ...
                <div className="pt-3 border-t border-dashed flex justify-between items-center">
                   <span className="text-[10px] text-muted font-bold italic">{alert.affectedEntity}</span>
                   <button onClick={() => navigate(`/reports/analytics?entity=${alert.affectedEntity}`)} className="text-[10px] text-primary font-black uppercase tracking-widest flex items-center gap-1 hover:underline">
            ...
            {(!alerts || alerts.length === 0) && (
              <div className="py-16 text-center space-y-4">
                ...
                <div>
                   <p className="text-body font-black text-primary uppercase tracking-widest">Hệ thống OK</p>
                   <p className="text-small text-muted italic">Không ghi nhận bất thường 24h qua</p>
                </div>
              </div>
            )}
          </div>
