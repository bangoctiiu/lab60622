...
const RegisterPage = () => {
  ...
  if (isSuccess) {
    return (
      <div className="min-h-screen bg-bg flex items-center justify-center p-6 animate-in fade-in zoom-in duration-500">
        <div className="max-w-md w-full bg-white rounded-[40px] p-12 text-center shadow-2xl shadow-primary/10 border border-white">
          <div className="w-20 h-20 bg-success/10 text-success rounded-3xl flex items-center justify-center mx-auto mb-8 shadow-lg shadow-success/10">
            <CheckCircle2 size={40} className="stroke-[2.5]" />
          </div>
          <h1 className="text-display text-primary mb-4 text-3xl">Đăng ký thành công!</h1>
          <p className="text-body text-muted leading-relaxed mb-10">
            Cảm ơn bạn đã tham gia cộng đồng SmartStay. Vui lòng kiểm tra email để xác thực tài khoản trước khi đăng nhập.
          </p>
          <Link 
            to="/public/login" 
            className="btn-primary w-full flex items-center justify-center gap-2 py-4 shadow-xl shadow-primary/20"
          >
            Về trang đăng nhập <ArrowRight size={18} />
          </Link>
        </div>
      </div>
    );
        ...
        <div className="max-w-md w-full mx-auto space-y-10 py-12">
          ...
          <div className="pt-6 text-center">
            <p className="text-body text-muted font-bold">
              Đã có tài khoản?{' '}
              <Link to="/public/login" className="text-primary underline underline-offset-4 decoration-2 decoration-primary/30 hover:decoration-primary transition-all">
                Đăng nhập tại đây
              </Link>
            </p>
          </div>
